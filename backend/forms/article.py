...
class ArticleForm(django_forms.Form):
    ...
    article_type_id = django_fields.IntegerField(
        widget=django_widgets.Select(choices=models.Article.type_choices)
    )
    ...
    status = django_fields.CharField(
        widget=django_widgets.Select(choices=models.Article.status_choices)
    )
